...
while option=='Si':
    ...
    if TMin<5 or TMax>35:
       ETMayorOMin=1
       ErroresTMayOTMen.append(ETMayorOMin)

    if TMax==0 and TMin==0 == False:
        ListaTempereatura.pop(-1)
        ErroresTMenores.pop(-1)
        
        ErroresTMayOTMen.pop(-1)
        
        ListaTmin.pop(-1)
        ListaTMax.pop(-1)
        break

#X=ListaTempereatura.count((TMax>35),(TMin<5))

#for i in ListaTempereatura:
#    if i>35 or 
...
print('El numero de días en total que se tuvieron temperaturas con error son: ', len(ErroresTMayOTMen))
...
perrores2=(100*(len(ErroresTMayOTMen)))/(len(ListaTempereatura))
